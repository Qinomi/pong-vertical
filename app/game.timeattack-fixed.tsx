...
export default function GameScreen() {
    ...
    const firstToParam = params.firstTo;
    const firstToStr = Array.isArray(firstToParam) ? firstToParam[0] : (firstToParam as string | undefined);
    const parsedFirstTo = firstToStr ? parseInt(firstToStr, 10) : NaN;
    const firstTo = Number.isFinite(parsedFirstTo) ? Math.max(1, Math.min(99, parsedFirstTo)) : undefined;
    ...
    const isTimeAttack = result?.mode === 'TIME_ATTACK';
    const survivedSeconds = isTimeAttack
        ? (typeof result?.timeSpent === 'number' ? result.timeSpent : (result?.playerScore ?? 0))
        : 0;

    const didWin = isTimeAttack
        ? survivedSeconds >= TIME_ATTACK_DURATION
        : (result ? result.playerScore > result.aiScore : false);

    const yourLabel = isTimeAttack ? 'SECONDS SURVIVED' : 'YOUR SCORE';
    const aiLabel = isTimeAttack ? 'BREACHES' : 'AI SCORE';
                    ...
                    <LinearGradient colors={['#1a1a2e', '#16213e']} style={styles.menuBox}>
                        <View style={styles.winnerHeader}>
                            <Ionicons
                                ...
                                size={60}
                                color={didWin ? "#FFD700" : "#ff4b2b"}
                            />
                            <Text style={[styles.winnerTitle, { color: didWin ? "#00f3ff" : "#ff4b2b" }]}>
                                {didWin ? "PLAYER WINS!" : "AI DOMINANCE"}
                            </Text>
                        </View>

                        <View style={styles.summaryScores}>
                            <View style={styles.sumRow}>
                                <Text style={styles.sumLabel}>{yourLabel}</Text>
                                <Text style={[styles.sumVal, { color: '#00f3ff' }]}>{result?.playerScore}</Text>
                            </View>
                            <View style={styles.sumRowDivider} />
                            <View style={styles.sumRow}>
                                <Text style={styles.sumLabel}>{aiLabel}</Text>
