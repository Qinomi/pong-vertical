...
type Overlay = 'NONE' | 'MODE' | 'FIRST_TO_X_MENU' | 'FIRST_TO_X_INPUT' | 'RANDOMIZING';
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      {[...Array(20)].map((_, i) => (
        <View
          key={`v-${i}`}
          style={[styles.gridLineV, { left: (SCREEN_WIDTH / 10) * i }]}
        />
      ))}
      {[...Array(40)].map((_, i) => (
        <View
          key={`h-${i}`}
          style={[styles.gridLineH, { top: (SCREEN_HEIGHT / 20) * i }]}
        />
      ))}
    </View>
  );
...
export default function MenuScreen() {
  ...
  const isModeSelect = overlay === 'MODE';
  const isFirstToXMenu = overlay === 'FIRST_TO_X_MENU';
  const isFirstToXInput = overlay === 'FIRST_TO_X_INPUT';
  const isRandomizing = overlay === 'RANDOMIZING';
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal visible={isModeSelect} transparent animationType="slide">
      ...
      {/* FIRST TO X MENU */}
      <Modal visible={isFirstToXMenu} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.menuBoxWide}>
            <LinearGradient colors={['#1a1a2e', '#16213e']} style={styles.menuInnerWide}>
              <View style={styles.modalHeader}>
                ...
                <TouchableOpacity onPress={() => setOverlay('MODE')}>
                  <Ionicons
                    name="close-circle"
                    size={32}
                    color="rgba(255,255,255,0.3)"
              ...
              <Text style={styles.smallHint}>
                Pick a win threshold, or let the arena decide (3–20).
              </Text>

              <TouchableOpacity
                style={styles.primaryBtn}
                onPress={() => setOverlay('FIRST_TO_X_INPUT')}
              >
      ...
      <Modal visible={isFirstToXInput} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.menuBoxWide}>
            <LinearGradient colors={['#1a1a2e', '#16213e']} style={styles.menuInnerWide}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>SET X</Text>
                <TouchableOpacity onPress={() => setOverlay('FIRST_TO_X_MENU')}>
                  <Ionicons
                    name="close-circle"
                    size={32}
                    color="rgba(255,255,255,0.3)"
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TouchableOpacity
                  ...
                  onPress={() => setOverlay('FIRST_TO_X_MENU')}
                >
